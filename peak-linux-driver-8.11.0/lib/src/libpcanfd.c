...
int pcanfd_set_device_id(int fd, __u32 devid)
{
	...
	return -__errno_ioctl(fd, PCAN_EXTRA_PARAMS, &ep);
}
...
int pcanfd_get_device_id(int fd, __u32 *pdevid)
{
	...
	err = -__errno_ioctl(fd, PCAN_EXTRA_PARAMS, &ep);
	if (!err)
#ifndef USES_32BITS_DEVICENO
...
/*
 * int pcan_set_extra_params(int fd, const struct pcan_extra_params *pe)
 *
 *	Set/get extra parameters to a device (if supported).
 */
int pcan_set_extra_params(int fd, const struct pcan_msg_filter *pe)
{
#ifdef DEBUG
...
int pcanfd_open(const char *dev_pcan, __u32 flags, ...)
{
	...
	if (flags & OFD_NONBLOCKING) {
		...
		err = fcntl(fd, F_SETFL, f | O_NONBLOCK);
		if (err) {
			err = -errno;
#ifdef DEBUG
			__fprintf(stddbg,
				"%s(%s): fcntl() failed (err %d)\n",
...
#ifdef DEBUG
		__fprintf(stddbg,
			"%s(%s): pcanfd_set_init() failed (err %d)\n",
			__func__, dev_pcan, err);
#endif
		goto open_failure;
	}
