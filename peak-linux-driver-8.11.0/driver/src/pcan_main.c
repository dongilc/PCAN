					...
					struct pcanfd_rxmsg *pqm)
{
	...
	if (pqm->hwtv.ts_mode == PCANFD_OPT_HWTIMESTAMP_OFF) {
		/* in that case, hwtv.tv contains host time of the event */
	...
	/* add number of µs since last sync */
...
#ifdef DEBUG_TS_DECODE
	pr_info(DEVICE_NAME
		": %s(%s, ts_us=%llu): ts_mode=%d sync_ts_us=%llu\n",
		__func__, dev->adapter->name, hwtv->ts_us, dev->ts_mode,
		dev->time_sync.ts_us);
#endif
	switch (dev->ts_mode) {
	default:
...
#if 0
	/* to avoid 2^32 wrapping problem with us counter in case of inactivity,
	 * reset sync system if previous sync_times() was made more than an hour
	 * ago.
	 */
	if (dtv_us > 3600*USEC_PER_SEC)
		goto lbl_reset;
#endif
 ...
 *
 * If pdev not NULL, pdev->adapter MUST be defined before.
 */
int pcan_find_free_minor(struct pcandev *pdev, int from, int until)
{
	...
	while (1) {
		struct pcandev *dev = pcan_is_minor_used(pdev, minor);
		int inc = 1;

		/* if minor not used, it's ok */
		if (!dev)
			break;

		/* if pdev doesn't belong to the same adapter than dev, then
		 * jump over can_count slots to ensure that devices of that
		 * adapter will always be sequential.
		 * Otherwise (pdev belongs to the same adapter), then
		 * continue with next minor...
		 */
		if (pdev)
			if (pdev->adapter != dev->adapter)
				inc = dev->adapter->can_count;

		/* check whether next value is used or not */
		minor += inc;
...
#ifdef SYSFS_SUPPORT
static ssize_t version_show(struct class *cls,
...
static ssize_t clk_ref_show(struct class *cls, struct class_attribute *attr,
			    char *buf)
{
...
static int _pcan_set_ts_mode(struct pcandev *dev, u32 ts_mode, bool fallback)
{
	...
	if (dev->features & PCAN_DEV_TS_SOF_RDY) {
		if (sof_flag) {
			if (!ucan_set_options(dev, UCAN_OPTION_TS_SOF))
				dev->flags |= PCAN_DEV_TS_SOF;

		} else {
			if (!ucan_clr_options(dev, UCAN_OPTION_TS_SOF))
				dev->flags &= ~PCAN_DEV_TS_SOF;
		}
...
int init_module(void)
{
	...
	default:
		pr_err(DEVICE_NAME ": invalid clk ref value %u; "
		       "switch back to default one (%u)\n",
