...
int pcan_netdev_rx(struct pcandev *dev, struct pcanfd_rxmsg *pqm)
{
	...
	pr_info(DEVICE_NAME ": %s(id=%xh flgs=%xh dlc=%u "
		"[%02x %02x %02x %02x %02x %02x %02x %02x] "
		") < %s CAN%u\n",
		__func__, pf->id, pf->flags, pf->data_len,
	...
	case PCANFD_TYPE_CANFD_MSG:
		...
		/* Note: use deftsmode and drvclkref module parameters to
		 * control how hw timestamps are given to the network layer.
		 * Of course, if deftsmode == PCANFD_OPT_HWTIMESTAMP_OFF(0) then
		 * it's not possible to give any hardware timestamp here.
		 */
		if ((pf->flags & PCANFD_TIMESTAMP) &&
		    (dev->features & PCAN_DEV_HWTS_RDY) &&
		    (pqm->hwtv.ts_mode != PCANFD_OPT_HWTIMESTAMP_OFF)) {
	...
	case PCANFD_TYPE_ERROR_MSG:
		...
		pcf->can_id = CAN_ERR_FLAG | CAN_ERR_PROT | CAN_ERR_BUSERROR;
		ld = CAN_ERR_DLC;
		...
		/* set error location */
		pcf->data[3] = pf->data[0];

		/* Error occurred during transmission? */
		if (!(pf->flags & PCANFD_ERRMSG_RX))
			pcf->data[2] |= CAN_ERR_PROT_TX;

		pcf->data[6] = dev->tx_error_counter;
		pcf->data[7] = dev->rx_error_counter;
