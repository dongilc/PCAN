 ...
 *
 * To link the 3 USB chip tohether, then:
 * - bus must be identical
 * - parent.portnum must be identical
 * - portnum must be sequential
 *
...
int pcan_usbfd_init(struct pcan_usb_interface *usb_if)
{
...
#if 0
	/* Donot rely on calibration msgs timestamps any more to sync clocks,
	 * since these timestamps reset each time interface is put UP or
	 * USB X6 is unplugged.
	 *
	 * Clock sync is now based on event timestamps only:
	...
	case PCAN_USBX6_PRODUCT_ID:
		...
		/* if no usb_if has been found, create one and consider it
		 * as the 1st one of the adapter
		 */
		if (!same_if) {
			usb_if->adapter = pcan_alloc_adapter(str_pcan_usb_x6,
						pcan_usbx6_devices++,
						6);
		/* Otherwise, an usb_if is already registered with previous
		 * portnum: sibling found!
		 */
		} else {
