...
#define PCAN_USBPRO_OFFSET_OK			0x00000002UL
...
#ifndef MSGTYPE_SINGLESHOT
#define MSGTYPE_SINGLESHOT		PCANFD_MSG_SNG
#endif
					...
					u32 ts_low, struct pcan_timeval *tv)
{
	...
	if (dev->wType == DEBUG_TS_HWTYPE)
		...
		pr_info(DEVICE_NAME
			": %s CAN%u -> MSG[ts=%u]: "
			"usb_if[uptime_us=%llu can_ts_low=%u]\n",
			dev->adapter->name, dev->can_idx+1,
			ts_low, usb_if->uptime_us, usb_if->can_ts_low);
#endif

	/* handle 32 bit wrapping: can't simply inc can_ts_high because we
	 * have to take into account inactivity periods larger than k * 2^32 us.
	 * Use uptime_us to fix can_ts_high.
	 */
	if (ts_low < usb_if->can_ts_low) {

		/* Note: must be sure that uptime_us has been adjusted before */
		if (usb_if->state & PCAN_USBPRO_OFFSET_OK) {
			u64 d;

			/* use uptime_us to find ts_us */
			usb_if->can_ts_high = (usb_if->uptime_us >> 32) + 1;

			ts_us = ((u64 )usb_if->can_ts_high << 32) + ts_low;

			/* Sure ts_us > uptime_us.
			 * Check if (ts_us - uptime_us) is not too large.
			 * If it is, then can_ts_high should be decremented.
			 * Never ts_us can be more than SYNC_PERIOD from
			 * uptime_us. Even more, a wrong ts_high will generate
			 * d >= 2^32.
			 */
			d = ts_us - usb_if->uptime_us;
			if (d > 2*PCAN_USB_FD_SYNC_PERIOD) {
				/* hmmm... Ok! ts_high should be decremented */
				usb_if->can_ts_high--;
			}
	...
	/* Otherwise, first event received is used to start time sync
	 * (since CM ts reset to 0 each time the option is requested, while
	 * events ts don't)
	 */
	} else if (!(usb_if->state & PCAN_USBPRO_OFFSET_OK) &&
							usb_if->uptime_us) {
		int i;
		u32 dus;

#ifdef DEBUG_TIMESTAMP
		pr_info(DEVICE_NAME ": %s(%s CAN%u): resync CM ts with msg ts: "
			"uptime_us=%llu is set to ts=%llu\n",
			__func__, dev->adapter->name, dev->can_idx+1,
			usb_if->uptime_us, ts_us);
#endif
		/* Start syncing clocks. Next syncs will be done by calibraton
		 * msgs.
		 */
		pcan_sync_times64(dev, ts_us, 0);
		...
		for (i = 0; i < usb_if->can_count; i++) {
			struct pcandev *devi = usb_if_dev(usb_if, i);
			if (devi != dev)
				memcpy(&devi->time_sync, &dev->time_sync,
					sizeof(dev->time_sync));
		}

		/* adjust uptime too (ts_low is the only time we have that
		 * corresponds to the moment the device has been powered up)
		 */
		dus = timeval_to_us(&dev->time_sync.tv) -
					timeval_to_us(&usb_if->tv_sync);
		usb_if->uptime_us = ts_us - dus;

		usb_if->state |= PCAN_USBPRO_OFFSET_OK;
	}
					...
					u32 ts_low, u32 dev_frame_index)
{
	...
	if (usb_if->dev_frame_index < 0) {
		...
		usb_if->can_ts_low = PCAN_TS_LOW_INIT;
	...
	/* Must consider that timestamps in calibration msgs restart from 0,
	 * while event timestamps don't. If calibration msgs are always
	 * received by the driver during the entire life of the device (that is,
	 * from probing time to disconnect time), then use these msgs as a
	 * periodic clock that gives timesamps, and add the difference between
	 * two timestamps into an "uptime" counter.
	 *
	 * uptime_us is the operating time of the device and the driver, i.e.
	 * the time during which it is powered and handled by the driver.
	 * It starts from 0 each time the driver is loaded and grows up while
	 * the device is connected. It is also used to compute the high 32 bit
	 * value of event timestamps.
	 */
	...
	/* Since time sync MUST be based on events timestamps, then wait for
	 * 1st event to set its own timestamp as timestamp base.
	 * Use that periodic clock to compute uptime_us. Keep local time while
	 * no event has been received to estimate new uptime_us according to
	 * 1st event ts base.
	 */
	if (!(usb_if->state & PCAN_USBPRO_OFFSET_OK)) {
		pcan_gettimeofday(&usb_if->tv_sync);
		return 0;
	}
...
#endif
		if (!usb_if->index)
		pr_info(DEVICE_NAME ": %s [CM]: usb_if[ts=%llu] dfi=%d\n",
			usb_if->adapter->name, usb_if->uptime_us, dfi);
#endif

	/* handle time sync in dev[0] even if it is not opened
	 * -50 µs gives times ~ those with PCIe FD for the USB FD devices
	 */
	if (pcan_sync_times64(dev0, usb_if->uptime_us, dfi))
