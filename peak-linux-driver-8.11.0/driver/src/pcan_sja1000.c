...
static int __sja1000_write(struct pcandev *dev, struct pcan_udata *ctx)
{
	...
	int err = pcan_fifo_get(&dev->writeFifo, &tx);
	if (!err)
...
int sja1000_probe(struct pcandev *dev)
{
	...
	if (tmp & 0x10) {
		pr_err(DEVICE_NAME ": %s(): Abnormal CLKDIVIDER %02Xh\n",
			__func__, tmp);
		goto fail;
	}
	...
	if ((tmp & 0x30) != 0x30) {
		pr_err(DEVICE_NAME ": %s(): Abnormal SJA1000_SR %02xh\n",
			__func__, tmp);
		goto fail;
	}
	...
	if (tmp & 0xfb) {
		pr_err(DEVICE_NAME ": %s(): Abnormal SJA1000_IR %02xh\n",
			__func__, tmp);
		goto fail;
	}
	...
	if (tmp) {
		pr_err(DEVICE_NAME
			": %s(): Abnormal RECEIVE_MSG_COUNTER %02xh\n",
			__func__, tmp);
		goto fail;
	}
