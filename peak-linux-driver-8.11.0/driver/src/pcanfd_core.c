...
static int pcanfd_check_init(struct pcandev *dev, struct pcanfd_init *pfdi)
{
	 ...
	 * match the device default clock.xi
	 */
	else {
				      ...
				      struct pcanfd_init *pfi2)
{
	...
	if (pfi1->flags & PCANFD_INIT_USER) {
		err = pcanfd_check_init(dev, pfi1);
		if (err)
			return err;

		/* no need to check them next */
		pfi1->flags &= ~PCANFD_INIT_USER;
	}

	if (pfi2->flags & PCANFD_INIT_USER) {
		err = pcanfd_check_init(dev, pfi2);
		if (err)
			return err;

		/* no need to check them next */
		pfi2->flags &= ~PCANFD_INIT_USER;
	}
...
int pcanfd_ioctl_set_init(struct pcandev *dev, struct pcanfd_init *pfdi)
{
	...
	if (!err) {
		/* bittiming are equal. Don't need to change the controller
		 * state. Just update the device init_settings fields with
		 * other initialization settings that don't deal with the
		 * controller itself.
		 */
		pcanfd_copy_init(&dev->init_settings, pfdi);
		return 0;
	...
	if (err > 0)
		pr_info(DEVICE_NAME
			": %s(CAN%u): bittiming settings differ (err %d)\n",
			dev->adapter->name, dev->can_idx+1, err);
#endif
#endif
