...
(JNIEnv *env, jobject obj, jobject Channel, jobject Parameter, jobject Buffer, jint BufferLength)
{
	...
	// Check Buffer's type
	if ((*env)->IsInstanceOf(env, Buffer, (*env)->FindClass(env, JAVA_CLASS_PEAK_MUTABLE_INTEGER)) == JNI_TRUE)
	{
		int intBuffer = 0;
		//Call PCANBasic Function
	...
	else if ((*env)->IsInstanceOf(env, Buffer, (*env)->FindClass(env, JAVA_CLASS_JRE_STRINGBUFFER)) == JNI_TRUE)
	{
		char charBuffer[256] = "";
		//Call PCANBasic Function
	...
	else if ((*env)->IsInstanceOf(env, Buffer, (*env)->FindClass(env, JAVA_CLASS_PEAK_MUTABLE_LONG)) == JNI_TRUE)
	{
		long long longBuffer = 0;
		//Call PCANBasic Function
		status = PCBasic_GetParameter(pChannel, pParameter, &longBuffer, sizeof(longBuffer));
		//Parse int Result Into Java Integer To Maintain Reference
		cls = (*env)->FindClass(env, JAVA_CLASS_PEAK_MUTABLE_LONG);
		mid = (*env)->GetMethodID(env, cls, "setValue", "(J)V");
		(*env)->CallVoidMethod(env, Buffer, mid, longBuffer);
		(*env)->DeleteLocalRef(env, cls);
	}
	else if ((*env)->IsInstanceOf(env, Buffer, (*env)->FindClass(env, "[L"JAVA_CLASS_PEAK_TPCANChannelInformation";")) == JNI_TRUE)
	{
		jobject chInfoObj;
