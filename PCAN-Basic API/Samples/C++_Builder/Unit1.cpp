...
void TForm1::ProcessMessage(TPCANMsg theMsg, TPCANTimestamp itsTimeStamp)
{
    ...
    newTimestamp = itsTimeStamp.micros + 1000 * itsTimeStamp.millis + 0x100000000UI64 * 1000 * itsTimeStamp.millis_overflow;
...
void __fastcall TForm1::btnParameterGetClick(TObject *Sender)
{
    ...
    case 0:
        ...
        if (stsResult == PCAN_ERROR_OK)
        {
            info = Format("The configured Device-ID is %d", ARRAYOFCONST((iBuffer)));
            IncludeTextMessage(info);
        }
        break;
